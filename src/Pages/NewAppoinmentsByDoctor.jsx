...
//       setaddNew(false);
//       setPatientDetails({ ...data, id: res.patienttableId });
//       setStep(4);
//     } catch (error) {
...
//   const user = await GET(`get_user/${user.userId}`);
...
//   const addAppointment = async () => {debugger;
...
const Step3 = ({ setPatientDetails, setStep }) => {
  ...
  const onSubmit = async (data) => {
    let apiData = {
      ...
      isd_code: isd_code,
      user_id: user.id,
      patienttableId: user.userId
    };

    try {
      setisUserAddLoading(true);
      const res = await ADD(user.token, "add_family_member-by-doc", apiData);

      showToast(toast, "success", "Success");
      QueryClient.invalidateQueries("patients");
      setaddNew(false);
      setPatientDetails({ ...data, id: res.id,patienttableId});
      setStep(4);
    } catch (error) {
      setisUserAddLoading(false);
      //showToast(toast, "error", "something went wrong");
